...
final class RecurringTransactionManager {
  ...
  /// Creates every not-yet-materialised installment of ONE series, synchronously.
  ///
  /// Purchases materialise only their first three installments; the rest appear as the user browses
  /// forward (`generateInstancesLazilyForMonths`). Early payment and cancellation have to reason about
  /// the WHOLE remaining series at once, and a row that does not exist cannot be offered or refunded —
  /// so a 10× purchase would silently offer three and quietly under-refund the other seven.
  ///
  /// Synchronous and series-scoped on purpose: the caller is about to read the series back and needs
  /// the rows to be there, and generating one series is far cheaper than a month-wide pass. Shares the
  /// date and amount rules with the lazy generator above rather than restating them, so the two cannot
  /// disagree about what installment #7 costs or falls on.
  ///
  /// - Returns: how many rows were created.
  @discardableResult
  func materializeInstallmentSeries(parentId: Int) -> Int {
    ...
    let existingAnchors = Set(
      all.filter { $0.parentTransactionId == parentId }.map { $0.budgetMonthDate })

    // An instance the user deliberately deleted must stay deleted — re-creating it here would undo
    // that on the next visit to the early-payment screen.
    operationLock.lock()
    let excludedAnchors = deletedInstanceAnchors[parentId] ?? []
    ...
    for installmentNumber in 1...totalInstallments {
      ...
      let targetAnchor = targetDate.monthAnchor
      guard !existingAnchors.contains(targetAnchor), !excludedAnchors.contains(targetAnchor) else {
        continue
      }
      ...
      let installmentModel = TransactionModel(
        title: cleanTitle,
        category: parent.category.key,
        amount: installmentNumber == 1 ? amountPerInstallment + remainder : amountPerInstallment,
        type: parent.type.key,
        dateTimestamp: Int(installmentDate.timeIntervalSince1970),
        budgetMonthDate: targetAnchor,
        parentTransactionId: parentId,
      ...
      do {
        ...
        logError("Error materializing installment \(installmentNumber) of series \(parentId): \(error)")
      }
    }

    if created > 0 { TransactionRepository.invalidateCache() }
    return created
  }
