...
struct RecurringSeriesLinker {
  ...
  func repairTransactionSeries(around parentId: Int) -> Int {
    ...
    let scopeById: [Int: String] = [:]  // no group ledgers on this branch
    ...
    // No series-level notification manager on this branch; per-row reminders are rescheduled by
    // the repository's own update paths.
  ...
  func repairAllocationSeries(around parentId: Int) -> Int {
    ...
    let targetKey = AllocationSeriesFingerprint(
      scope: nil, category: parent.category.key)
    ...
    for candidate in all.sorted(by: { $0.monthDate < $1.monthDate }) {
      ...
      guard
        AllocationSeriesFingerprint(scope: nil, category: candidate.category.key)
          == targetKey
      else { continue }
