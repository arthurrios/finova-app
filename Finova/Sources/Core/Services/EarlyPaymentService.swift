...
final class EarlyPaymentService {
    ...
    ///
    /// `SecureLocalDataManager` first, because that is what scopes every read in
    ...
    func releaseInstallments(settledBy paymentId: Int) {
        ...
        reconcileNotifications(for: installmentIds)
    }
    ...
    private func finishMutation(
        ...
        reconcileNotifications(for: [paymentId] + installmentIds)

        NotificationCenter.default.post(name: .transactionDataChanged, object: nil)
    }

    /// An anticipated installment must stop reminding the user about a payment already made, and the
    /// debit that replaced it needs a reminder of its own. Both directions go through the repository's
    /// reconcile, which skips settled installments and reschedules everything else from current data.
    private func reconcileNotifications(for transactionIds: [Int]) {
        for id in Set(transactionIds) {
            transactionRepo.reconcileNotification(transactionId: id)
        }
    }
}
