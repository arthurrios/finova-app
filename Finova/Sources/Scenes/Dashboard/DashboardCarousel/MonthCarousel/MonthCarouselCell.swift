...
class MonthCarouselCell: UICollectionViewCell {
    ...
    private func rebuildAllocationsUI(
        ...
        let resolvedAllocations =
            allocations
            ?? allocationService.getAllocationsWithUsage(forMonth: currentMonthAnchor, in: ledgerScope)
        let resolvedSpending =
            unallocatedSpending
            ?? allocationService.getUnallocatedCategoriesWithSpending(
                forMonth: currentMonthAnchor, in: ledgerScope)
        let resolvedSummary =
            summary
            ?? allocationService.getUnallocatedSummary(forMonth: currentMonthAnchor, in: ledgerScope)
