...
extension MonthSelectorView: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(
    ...
    // Calculate a reasonable width based on the text content
    let monthText = months[indexPath.item]
    let font = UIFont.systemFont(ofSize: 14)  // Use a reasonable font size
    let textSize = (monthText as NSString).size(withAttributes: [.font: font])
    let padding: CGFloat = 16  // Add some padding
