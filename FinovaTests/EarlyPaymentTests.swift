...
final class EarlyPaymentTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        SecureLocalDataManager.shared.authenticateUser(firebaseUID: "test_early_payment_\(UUID().uuidString)")
    ...
    private func makeCard() -> CreditCard? {
        ...
        let id = repo.insertCard(
            CreditCard(
                ...
                cardColor: .blue, userId: SecureLocalDataManager.shared.getCurrentUserUID() ?? "",
    ...
    func testAnticipatedInstallmentLeavesItsOwnStatementTotal() throws {
        ...
        let before = try DBHelper.shared.getTransactionSumForStatement(statementId: statementId)
        XCTAssertEqual(
        ...
        let after = try DBHelper.shared.getTransactionSumForStatement(statementId: statementId)
        XCTAssertEqual(
            after, 0,
