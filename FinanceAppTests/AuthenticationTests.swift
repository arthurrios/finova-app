...
class AuthenticationTests: XCTestCase {
  ...
  func testAuthenticationManagerErrorHandling() {
    print("🧪 Testing AuthenticationManager error handling")

    let authManager = AuthenticationManager.shared
    var capturedError: Error?

    let errorExpectation = XCTestExpectation(description: "Authentication Error")

    let errorDelegate = MockAuthenticationDelegate(
      onSuccess: { user in
      ...
      onFailure: { error in
        capturedError = error
        print("🔍 Captured authentication error: \(error.localizedDescription)")
        errorExpectation.fulfill()
      }
    )

    authManager.delegate = errorDelegate
    currentTestDelegate = errorDelegate

    // Test with non-existent user login
    let nonExistentEmail = "nonexistent+\(Int(Date().timeIntervalSince1970))@example.com"

    print("🔍 Attempting to sign in with non-existent email: \(nonExistentEmail)")
    authManager.signIn(email: nonExistentEmail, password: "password123")

    wait(for: [errorExpectation], timeout: 15.0)

    XCTAssertNotNil(capturedError, "Should capture authentication error")
    if let error = capturedError {
      ...
      // Verify it's a proper Firebase auth error - be more flexible with validation
      let errorString = error.localizedDescription.lowercased()
      let nsError = error as NSError

      let isFirebaseError =
        nsError.domain.contains("Firebase") || nsError.domain.contains("FIRAuth")
        || errorString.contains("firebase") || errorString.contains("authentication")
        || errorString.contains("user") || errorString.contains("password")
        || errorString.contains("email") || errorString.contains("invalid")
        || errorString.contains("credential") || nsError.code == 17011  // FIRAuthErrorCodeUserNotFound
        || nsError.code == 17009  // FIRAuthErrorCodeWrongPassword

      print("🔍 Is Firebase error: \(isFirebaseError)")
      XCTAssertTrue(
        isFirebaseError,
        "Should be a Firebase authentication error. Domain: \(nsError.domain), Code: \(nsError.code), Description: \(error.localizedDescription)"
      )
    }

    print("✅ AuthenticationManager error handling test completed")
  }
