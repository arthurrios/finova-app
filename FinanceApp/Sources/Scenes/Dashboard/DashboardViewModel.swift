...
final class DashboardViewModel {
    ...
    func scheduleAllTransactionNotifications() {
        ...
        notificationCenter.getNotificationSettings { settings in
            guard settings.authorizationStatus == .authorized else {
                print(
                    "❌ Notifications not authorized. Current status: \(settings.authorizationStatus.rawValue)"
                )
                return
            }
    ...
    private func _scheduleAllTransactionNotifications() {
        ...
        print("📅 Scheduling notifications for \(futureTxs.count) future transactions")
        
        futureTxs.forEach { scheduleNotification(for: $0) }
    ...
    private func scheduleNotification(for tx: Transaction) {
        ...
        guard let transactionId = tx.id else {
            print("❌ Cannot schedule notification for transaction without ID")
            return
        }
        ...
        guard tx.date > Date() else {
            print("⚠️ Skipping notification for past transaction: \(tx.title)")
            return
        }
        ...
        guard let scheduledDate = calendar.date(from: comps), scheduledDate > Date() else {
            print("❌ Invalid date components for transaction: \(tx.title)")
            return
        }
        ...
        notificationCenter.add(request) { error in
            if let error = error {
                ...
                print("✅ Scheduled notification for '\(tx.title)' on \(scheduledDate)")
            }
        }
    }
        ...
        notificationCenter.getPendingNotificationRequests { requests in
            ...
            for request in requests {
                ...
                if let trigger = request.trigger as? UNCalendarNotificationTrigger {
                    let dateComps = trigger.dateComponents
                    print("🔔 Notification '\(identifier)': \(title) - \(body)")
                    print(
                        "   📅 Scheduled for: \(dateComps.year ?? 0)/\(dateComps.month ?? 0)/\(dateComps.day ?? 0) at \(dateComps.hour ?? 0):\(dateComps.minute ?? 0)"
                    )
                } else {
                    print("🔔 Notification '\(identifier)': \(title) - \(body)")
                }
